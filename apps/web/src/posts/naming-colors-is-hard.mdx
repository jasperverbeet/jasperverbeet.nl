...
At the time of writing, the following image describes the color roles for the light theme on this website excluding states, I'll get to those later.
...
For default styles I like to omit the `default` state, because it's the most common state. In Tailwind we can achieve this by using the `DEFAULT` key. So

```tsx
<p class="text-text-primary-default" />
// becomes
...
By the way, the code for this website is open source, you can find my color role configuration [here](https://github.com/jasperverbeet/jasperverbeet.nl/blob/main/apps/web/tailwind.config.ts).
